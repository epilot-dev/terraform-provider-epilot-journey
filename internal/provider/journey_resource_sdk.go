...
func (r *JourneyResourceModel) ToSharedJourneyCreationRequest() *shared.JourneyCreationRequest {
	...
	for _, logicsItem := range r.Logics {
		...
		logics = append(logics, shared.Logics{
			Actions:         actions,
	...
	var rules []shared.Rules = nil
	for _, rulesItem := range r.Rules {
		source := rulesItem.Source.ValueString()
		sourceType := shared.SourceType(rulesItem.SourceType.ValueString())
		target := rulesItem.Target.ValueString()
		typeVar := shared.Type(rulesItem.Type.ValueString())
	...
	if r.Settings != nil {
		...
		if r.Settings.EmbedOptions != nil {
			...
			if !r.Settings.EmbedOptions.Lang.IsUnknown() && !r.Settings.EmbedOptions.Lang.IsNull() {
				*lang = shared.Lang(r.Settings.EmbedOptions.Lang.ValueString())
			} else {
				lang = nil
			}
			mode := new(shared.Mode)
			if !r.Settings.EmbedOptions.Mode.IsUnknown() && !r.Settings.EmbedOptions.Mode.IsNull() {
				*mode = shared.Mode(r.Settings.EmbedOptions.Mode.ValueString())
			} else {
				mode = nil
			}
			...
			embedOptions = &shared.EmbedOptions{
				Button:      button,
				Lang:        lang,
				Mode:        mode,
		...
		var runtimeEntities []shared.RuntimeEntities = nil
		for _, runtimeEntitiesItem := range r.Settings.RuntimeEntities {
			runtimeEntities = append(runtimeEntities, shared.RuntimeEntities(runtimeEntitiesItem.ValueString()))
		}
	...
	for _, stepsItem := range r.Steps {
		...
		steps = append(steps, shared.Steps{
...
func (r *JourneyResourceModel) RefreshFromSharedJourneyResponse(resp *shared.JourneyResponse) {
